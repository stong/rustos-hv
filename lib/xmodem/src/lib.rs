...
impl<T: io::Read + io::Write> Xmodem<T> {
    ...
    fn expect_byte_or_cancel(&mut self, byte: u8, expected: &'static str) -> io::Result<u8> {
        let read_result = self.expect_byte(byte, expected);
    ...
    fn expect_byte(&mut self, byte: u8, expected: &'static str) -> io::Result<u8> {
        let read_result: u8 = self.read_byte(byte != CAN)?;
        if read_result == byte {
            return Ok(byte)
        } else {
    ...
    pub fn read_packet(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        if buf.len() < 128 {
            return ioerr!(UnexpectedEof, "buffer must be of length 128");
        }
        if !self.started {
            self.write_byte(NAK)?;
            self.started = true;
        ...
        let read_result = self.read_byte(true)?;
        if read_result == EOT {
            self.started = false;
            self.write_byte(NAK)?;
            self.expect_byte_or_cancel(EOT, "expected EOT at EOT-NAK")?;
            self.write_byte(ACK)?;
            return Ok(0); // apparently, return 0 means EOT.
        }
        if read_result != SOH {
            return ioerr!(InvalidData, "expected EOT or SOH at start of packet");
        }
        self.expect_byte_or_cancel(self.packet, "expected packet number")?;
        self.expect_byte_or_cancel(!self.packet, "expected complemented packet number")?;
        for i in 0..128 {
            buf[i] = self.read_byte(false)?;
        }
        if self.read_byte(false)? != get_checksum(&buf[..128]) {
    ...
    pub fn write_packet(&mut self, buf: &[u8]) -> io::Result<usize> {
        if !self.started {
            ...
            self.expect_byte(NAK, "expected NAK at start of packet")?;
            self.started = true;
            (self.progress)(Progress::Started);
        }
        if buf.len() == 0 { // apparently, empty buffer means EOT.
            self.write_byte(EOT)?;
            self.expect_byte_or_cancel(NAK, "expected NAK at EOT")?;
            self.write_byte(EOT)?;
            self.expect_byte_or_cancel(ACK, "expected ACK at EOT-NAK-EOT")?;
            return Ok(0);
        }
        if buf.len() < 128 {
            return ioerr!(UnexpectedEof, "buffer length is not 128 or 0");
        }
        self.write_byte(SOH)?;
        ...
        if read_result == NAK {
            (self.progress)(Progress::NAK);
            return ioerr!(Interrupted, "retransmission requested");
        }
        if read_result != ACK {
            return ioerr!(InvalidData, "expected NAK or ACK at end of packet");
        }
        (self.progress)(Progress::Packet(self.packet));
        self.packet = self.packet.wrapping_add(1);
        Ok(128)

...
impl Gpio<Uninitialized> {
    ...
    pub fn into_alt(self, function: Function) -> Gpio<Alt> {
        let yeehaw1: u8 = self.pin / 10;
        let yeehaw2: u8 = self.pin % 10;
        let reg = &mut self.registers.FSEL[yeehaw1 as usize];
        reg.write(reg.read() & !((0b111 as u32) << (3 * yeehaw2)) | (function as u32) << (3 * yeehaw2));
        self.transition()
    }
...
impl Gpio<Output> {
    ...
    pub fn set(&mut self) {
        let yeehaw1: u8 = self.pin / 32;
        let yeehaw2: u8 = self.pin % 32;
        self.registers.SET[yeehaw1 as usize].write(1 << yeehaw2);
    }
    ...
    pub fn clear(&mut self) {
        let yeehaw1: u8 = self.pin / 32;
        let yeehaw2: u8 = self.pin % 32;
        self.registers.CLR[yeehaw1 as usize].write(1 << yeehaw2);
    }
}
...
impl Gpio<Input> {
    ...
    pub fn level(&mut self) -> bool {
        let yeehaw1: u8 = self.pin / 32;
        let yeehaw2: u8 = self.pin % 32;
        (self.registers.LEV[yeehaw1 as usize].read() >> yeehaw2) & 1 == 1
    }
}
